      ...
      <main class="relative h-full w-full bg-[url('/images/jungle.webp')] bg-cover bg-center">
        ...
        <div class="no-scrollbar h-[calc(100vh-36px)] w-full overflow-y-scroll">
          ...
          <Show when={quizStore.quizContent.length > 0}>
            ...
            <div class="mt-5 grid w-full grid-cols-1 gap-1.5">
              <Show
                when={checked()}
                fallback={
                  <For each={choices()}>
                ...
                <For each={choices()}>
                  {(item, index) => (
                    <Show
                      ...
                      fallback={
                        <div
                          class={`${item.created_at === quizStore.quizRender.created_at ? "bg-green-400/90" : "bg-[#fffeff]"} mx-auto h-11 w-2/3 cursor-pointer select-none rounded-sm text-center font-sfpro text-5 font-500 leading-11 text-black shadow-md transition`}
                        >
                          {item.choice}
                        </div>
                      }
                    >
                      <div
                        class={`${item.created_at === quizStore.quizRender.created_at ? "bg-green-400/90" : "bg-[rgba(249,0,0,0.9)]"} mx-auto h-11 w-2/3 cursor-pointer select-none rounded-sm text-center font-sfpro text-5 font-500 leading-11 text-black shadow-md transition`}
                      >
                        {item.choice}
                      </div>
                    </Show>
