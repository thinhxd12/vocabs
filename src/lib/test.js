export const TickClass = ({
  ...
  const easeOutCubic = Extension.getExtension(
    Extension.Type.EASING_FUNCTION,
    "ease-out-cubic",
  );
  const easeOutSine = Extension.getExtension(
    Extension.Type.EASING_FUNCTION,
    "ease-out-sine",
  );
  ...
  const draw = (state) => {
    ...
    if (state.isInitialValue()) {
      ...
      // shaders
      const shadowTop = DOM.create(
        "span",
        "tick-flip-shadow-top tick-flip-shadow tick-flip-front",
      );
      const shadowBottom = DOM.create(
        "span",
    ...
    if (!state.animating) {
      ...
      const ease = Extension.getExtension(
        Extension.Type.EASING_FUNCTION,
        state.style.flipEasing,
      );

      const tick = () => {
        // find cards that require animation
        const cardsToAnimate = state.cards.filter(
          (card) => !card.done && !card.waiting,
        );
        ...
        cardsToAnimate.forEach((card) => {
          if (card.offset !== null) {
            card.progress =
              (performance() - card.offset) / state.style.flipDuration;
          }
        ...
        cardsToAnimate.reverse().forEach((card, index) => {
          ...
          if (
            previousCard &&
            card.visual_progress <= previousCard.visual_progress
          ) {
        ...
        state.cards.forEach((card, index) => {
          ...
          const shadowFrontProgress =
            1 - Math.abs(card.visual_progress - 0.5) * 2;
          const highlightBackProgress = 1 - (card.visual_progress - 0.5) / 0.5;
          ...
          if (
            cardAbove &&
            card.visual_progress > 0.5 &&
            card.visual_progress > 0
          ) {
        ...
        let dist = 1;
        cardsToAnimate.forEach((card) => {
          let d = Math.abs(card.visual_progress - 0.5);
          if (d < dist) {
            dist = d;
